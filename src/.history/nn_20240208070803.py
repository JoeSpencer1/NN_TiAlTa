...
def validation_FEM(yname, filename, testname, train_size):
    ...
    if train_size == 80:
        ...
        if train_size < 40:
            sigma = 0.2
            datafem.y[0,:] *= 1 + sigma * np.random.randn(len(datafem.y[0]))
        kf = ShuffleSplit(
    ...
    '''
    initializer = dde.nn.initializers.VarianceScalingStacked(scale=1.0, mode="fan_in",distribution="truncated_normal")
    mape = []
    iter = 0
    for train_index, test_index in kf.split(datafem.X):
        ...
        data = dde.data.DataSet(
            X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test, initializer = initializer
        )
