...
[VectorPostprocessors]
  [y_disp]
    type = NodalValueSampler
    variable = disp_y
    execute_on = FINAL  # Executes only at the final timestep
    block = 1
    sort_by = y
  []
[]
...
[Outputs]
  ...
  [./csv]
    type = CSV
    execute_on = 'final'  # Ensures the CSV is executed at the final time step
    # Specify the postprocessor to output
    columns = 'y_disp'  # This should match the name of the vector postprocessor
  [../]
[]
