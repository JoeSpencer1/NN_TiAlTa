...
def validation_one(yname, testname, trainname, n_hi, og_hi=0, lay=2, wid=32):
    ...
    for train_index, test_index in kf.split(tdatatrain.X):
        ...
        tdatatrain.X = np.vstack([tdatatrain.X, tdatatrain.X[np.random.choice(tdatatrain.X.shape[0], size=datatrain.X.shape[0], replace=True)]])
        tdatatrain.y = np.vstack([tdatatrain.y, tdatatrain.y[np.random.choice(tdatatrain.y.shape[0], size=datatrain.y.shape[0], replace=True)]])
