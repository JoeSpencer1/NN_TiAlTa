...
def mfnn(data, lay=2, wid=128, whigh = 0.5):
    x_dim, y_dim = 3, 1
    activation = "selu"
    initializer = "LeCun normal"
    regularization = ["l2", 0.01]
    ...
    model.set_weights = [1 - whigh, whigh]
...
def validation_two(yname, train_size, trainlow, trainhigh, testname):
    ...
    with open('Output.txt', 'a') as f:
        f.write('validation_two ' + yname + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)[0]) + ' ' + str(np.std(mape, axis=0)[0]) + ' ' + t2s(testname) + ' ' + t2s(trainlow) + ' ' + t2s(trainhigh) + '\n')
    print(mape)
...
def validation_three(yname, train_size, trainlow, trainhigh, trainexp, testname):
    ...
    with open('output.txt', 'a') as f:
        f.write('validation_three ' + yname + ' ' + str(train_size) + ' ' + str(np.mean(ape, axis=0)[0]) + ' ' + str(np.std(ape, axis=0)[0]) + ' ' + t2s(testname) + ' ' + t2s(trainlow) + ' ' + t2s(trainhigh) + ' ' + t2s(trainexp) + '\n')
