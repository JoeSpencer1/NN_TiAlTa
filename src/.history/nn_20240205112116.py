...
def validation_FEM(yname, filename, train_size):
    ...
    def normalize(vec):
        ...
        #std_dev = np.std(vec, axis=0)
        #ones = np.ones_like(vec)
        #return(((vec / mean) - ones) / (std_dev / mean)) + ones
        return (vec / mean)
    ...
    if train_size == 80:
        ...
        kf = ShuffleSplit(
            n_splits=int(train_size * 2), test_size=len(datafem.X) - train_size, random_state=0
            #n_splits=int(train_size), test_size=len(datafem.X) - train_size, random_state=0
            #n_splits=10, test_size=len(datafem.X) - train_size, random_state=0
        )
    else:
        sigma = 0.2
        datafem.y *= 1 + sigma * np.random.randn(len(datafem.y))
        # Assuming datafem.y has a shape of (97, 1)
        random_array = sigma * np.random.randn(len(datafem.y))  # Generate random array
        random_array = random_array.reshape(-1, 1)  # Reshape random array to match datafem.y
        # Now, you can perform the multiplication
        datafem.y *= (1 + random_array)
        kf = ShuffleSplit(
            n_splits=int(train_size * 2), test_size=len(datafem.X) - train_size, random_state=0
            #n_splits=int(train_size * 8), test_size=len(datafem.X) - train_size, random_state=0
        )
