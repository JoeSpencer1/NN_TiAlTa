...
def model_dao(C, WpWt, dPdh, nu, hm, nu_i=0.07, E_i=1100e9, typ='b'):
...
def model_chollacoop(C, WpWt, dPdh, nu, hm, theta=70.3, nu_i=0.0691, E_i=1143):
    C *= 1e9
    E_i *= 1e9
    hm *= 1e-6
    # cstar = 1.1957  # Conical
    ...
    for l, r in [[1e7, 1e9], [1e9, 5e9], [5e9, 1e10]]:
        if (Pitheta(theta, Estar / l) - C / l) * (
            Pitheta(theta, Estar / r) - C / r
        ) < 0:
            sigma_r = optimize.brentq(
                lambda x: Pitheta(theta, Estar / x) - C / x, l, r
            )
            break
    else:
        raise ValueError
    ...
    return E/1e9, Estar/1e9, n, sigma_y/1e9
...
def main():
    ...
    for i in range(len(df)):
        if i not in [0,4,5,8,9,11,12,15,18,21,22,23,24,25]: # Remove 2 indices that do not converge
            print(model_chollacoop(df['C (GPa)'][i], df['Wp/Wt'][i], df['dP/dh (N/m)'][i], df['nu'][i], df['hmax(um)'][i]))
    # print('Dao')
    # for i in range(len(df)):
