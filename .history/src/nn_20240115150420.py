...
def validation_two(yname, train_size, exp, low, hi, lay=2, wid=128):
    ...
    if train_size == 0:
        kf = ShuffleSplit(
            n_splits=10, test_size=len(datalow.X) - train_size, random_state=0
        )
        for train_index, test_index in kf.split(datalow.X):
            iter += 1
            print("\nCross-validation iteration: {}".format(iter))

            X_train, X_test = datalow.X[train_index], dataexp.X[test_index]
            y_train, y_test = datalow.y[train_index], dataexp.y[test_index]
...
def validation_three(yname, train_size, train, exp, lay=2, wid=128):
    dataexp = ExpData(train, yname)
    dataFEM = FEMData(yname)
    dataBerk = BerkovichData(yname)
    datatest = ExpData(exp, yname)
    ...
    if train_size == 0:
        ...
        for train_index, test_index in kf.split(dataexp.X):
            ...
            data = dde.data.MfDataSet(
                X_lo_train=dataFEM.X,
                X_hi_train=np.vstack((dataBerk.X, dataexp.X[train_index])),
                y_lo_train=dataFEM.y,
                y_hi_train=np.vstack((dataBerk.y, dataexp.y[train_index])),
