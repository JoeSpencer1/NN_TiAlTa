...
def validation_FEM(yname, filename, testname, train_size):
    ...
    if filename == testname:
        ...
        train_index = np.arange(len(datafem.y))
        test_index = np.arange(len(datatest.y))
        ...
        for train_index, val_index in kf.split(datafem.X):
            X_train, X_val = datafem.X[train_index], datafem.X[val_index]
            y_train, y_val = datafem.y[train_index], datafem.y[val_index]
            
            # Append the training and validation data to lists
    ...
    for train_index, test_index in train_set:
        ...
        if filename == testname:
            ...
            while len(datatest.y) < len(datatest.X):
                datatest.X=np.vstack((datatest.X, datatest.X))
                datatest.y=np.vstack((datatest.y, datatest.y))
