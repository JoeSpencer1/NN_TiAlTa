...
def nn(data, lay, wid):
    ...
    model = Sequential()
    model.add(Input(shape=(data.train_x.shape[1],)))
    for _ in range(lay):
        model.add(Dense(wid, activation=activation, kernel_initializer=initializer, kernel_regularizer=regularization))
    model.add(Dense(1, activation=None, kernel_initializer=initializer, kernel_regularizer=regularization))
