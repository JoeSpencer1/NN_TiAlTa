...
def validation_one(yname, testname, trainname, n_hi, og_hi=0, lay=2, wid=32):
    ...
    for train_index, test_index in kf.split(datatrain.X):
        ...
        tdatatrain.X = np.vstack([tdatatrain.X, datatrain.X[np.random.choice(tdatatrain.X.shape[0], size=max(train_index)-len(tdatatrain.X)+1, replace=True)]])
        tdatatrain.y = np.vstack([tdatatrain.y, datatrain.y[np.random.choice(tdatatrain.y.shape[0], size=max(train_index)-len(tdatatrain.y)+1, replace=True)]])

        if len(tdatatrain.X) == 0:
            print('abcsadfroaecdszxerqwerqwer')
