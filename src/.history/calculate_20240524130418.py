...
def model_new(C, dPdh, nu, Er, hm, hr, theta=70.3, nu_i=0.0691, E_i=1143, typ='b'):
...
for i in range(len(df)):
    if i not in [0, 23]: # Remove 2 indices that do not converge
        print(model_new(df['C (GPa)'][i], df['dP/dh (N/m)'][i], df['nu'][i], df['E* (GPa)'][i], df['hmax(um)'][i], df['hr(um)'][i]))
print('Old')
for i in range(len(df)):
    if i not in [25]: # Remove 1 indices that do not converge
        print(i, model_old(df['C (GPa)'][i], df['dP/dh (N/m)'][i], df['nu'][i], df['E* (GPa)'][i], df['hmax(um)'][i], df['hr(um)'][i]))