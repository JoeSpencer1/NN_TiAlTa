...
def validation_one(yname, testname, trainname, n_hi, og_hi=0, lay=2, wid=32):
    ...
    while len(tdatatrain.X) < len(datatrain.X):
        temp = datastack(datatrain, n_hi, og_hi)
        tdatatrain.X = np.vstack(tdatatrain.X,temp.X)
        tdatatrain.y = np.vstack(tdatatrain.y,temp.y)
    for train_index, test_index in kf.split(tdatatrain.X):
        iter += 1
