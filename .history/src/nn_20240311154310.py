...
def validation_three(yname, testname, trainexp, n_exp, trainhigh, n_hi, trainlow, n_lo, lay=2, wid=128):
    ...
    with open('output.txt', 'a') as f:
        f.write('validation_three ' + yname + ' ' + f'{np.mean(ape, axis=0)[0]:.2f}' + ' ' + str(np.std(ape, axis=0)[0]) + ' ' + t2s(testname) + ' ' + t2s(trainexp) + ' ' + str(n_exp) + ' ' + t2s(trainhigh) + ' ' + str(n_hi) + ' ' + t2s(trainlow) + ' ' + str(n_lo) + str(lay) + ' ' + str(wid) + '\n')
