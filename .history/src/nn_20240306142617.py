...
def mfnn(data, lay, wid):
    ...
    model = dde.Model(data, net)
    model.compile('adam', lr=0.0001, loss='MAPE', metrics=['MAPE', 'APE SD'], loss_weights=[0.5, 0.5])
...
def validation_two(yname, train_size, testname, trainhigh, trainlow, lay=2, wid=128):
    ...
    with open('Output.txt', 'a') as f:
        f.write('validation_two ' + yname + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)[0]) + ' ' + str(np.std(mape, axis=0)[0]) + ' ' + t2s(testname) + ' ' + t2s(trainhigh) + ' ' + t2s(trainlow) + ' ' + str(lay) + ' ' + str(wid) + '\n')
    print(mape)
