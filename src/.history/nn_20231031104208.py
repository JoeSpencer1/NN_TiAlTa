...
def validation_two(yname, trname1, trname2, tstname, type, train_size, lay=9, wid=32, angles=[]):
    ...
    print(mape)
    print(yname, 'validation_one ', trname1, ' ', tstname, ' ', str(train_size), ' ', np.mean(mape), ' ', np.std(mape))
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + trname1 + ' ' +  tstname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')
