...
def validation_FEM(yname, filename, testname, train_size):
    ...
    if filename == testname:
        ...
        kf = ShuffleSplit(
            n_splits=10, train_size=len(datafem.X), random_state=0
        )
        kf1 = ShuffleSplit(
            n_splits=10, test_size=len(datatest.X), random_state=0
        )
    
    
    mape = []
    iter = 0
    for train_index, _ in kf.split(datafem.X), test_index, _ in kf1.split(datatest.X):
        iter += 1
