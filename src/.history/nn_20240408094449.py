...
def validation_one(yname, testname, trainname, n_hi, n_vd=0.2, lay=2, wid=32):
    ...
    for train_index, test_index in kf.split(datatest.X):
        ...
        train_index = (train_index * len(datatrain.X) / len(datatest.X)).astype(int)
...
def validation_two(yname, testname, trainhigh, n_hi, trainlow, n_lo, v_lo=0, n_vd=0.2, lay=2, wid=128):
    ...
    if n_hi == 0:
        kf = ShuffleSplit(
            n_splits=10, train_size=n_lo, test_size=len(datalow.y) - n_lo, random_state=0
        )
        for train_index, test_index in kf.split(datatest.X):
            iter += 1
            print('\nCross-validation iteration: {}'.format(iter))
            train_index = (train_index * len(datalow.X) / len(datatest.X)).astype(int)
        ...
        for train_index, test_index in kf.split(datatest.y):
            iter += 1
            print('\nIteration: {}'.format(iter), flush=True)
            #train_index = train_index % len(datahigh.X)
            #test_index = test_index % len(datatest.X)
            for i in range(len(train_index)):
                train_index[i] = train_index[i] // (len(datatest.y) / len(datahigh.y))
                #test_index[i] = test_index[i] // len(datatest.y)

            lo_ind = np.random.choice(datalow.X.shape[0], size=n_lo, replace=False)
            data = dde.data.MfDataSet(
                X_lo_train=datalow.X[lo_ind],
                X_hi_train=datahigh.X[train_index],
                y_lo_train=datalow.y[lo_ind],
