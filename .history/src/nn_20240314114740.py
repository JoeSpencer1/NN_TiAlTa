...
def validation_three_hi(yname, testname, trainexp, n_exp, trainhigh, n_hi, trainlow, n_lo, typ='hi',lay=2, wid=128):
    datalow = FileData(trainlow, yname)
    datahigh = FileData(trainhigh, yname)
        ...
        for train_index, _ in kf.split(dataexp.X):
            ...
            if typ == 'hi':
                ...
                data = dde.data.MfDataSet(
                    ...
                    y_lo_train=np.vstack((datalow.X[np.random.choice(datalow.y.shape[0], size=n_lo, replace=False)],datahigh.y[np.random.choice(datahigh.y.shape[0], size=n_hi, replace=False)])),
