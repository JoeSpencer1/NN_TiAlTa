    ...
    if n_exp == 0:
        ...
        if typ == 'hi':
            for train_index, test_index in kf.split(datatest.X):
                ...
                ape.append(dde.utils.apply(mfnn, (data,lay,wid,))[0])
        else:
            for train_index, test_index in kf.split(datatest.X):
                iter += 1
                print('\nCross-validation iteration: {}'.format(iter))
...
def find_properties(expname, train_hi, train_lo, lay=2, wid=128):
    datalow = FileData(train_lo,'all')
    print('\n1\n')
    datahigh = FileData(train_hi,'all')
    print('\n2\n')
    dataexp = FileData(expname,'all', new=True)

    print('\nHey!!\n\n', datalow.X.shape, datahigh.X.shape, dataexp.X.shape)
    print('\n\n\n', datalow.y.shape, datahigh.y.shape)#, dataexp.y.shape)

    ape = []
    y = []
    if len(datahigh.X) > len(datalow.X):
        larger = datahigh
        smaller = len(datalow.X)
        typ = 'h'
    else:
        larger = datalow
        smaller = len(datahigh.X)
        typ = 'l'
    train_size = len(larger.X)-1

    kf = ShuffleSplit(n_splits=10, train_size=train_size, random_state=0)

    for train_index, _ in kf.split(larger.X):
        print("\nIteration: {}".format(len(ape)))
        print(train_index)
        ...
        data = dde.data.MfDataSet(
            ...
            X_hi_test=datahigh.X,
            y_hi_test=datahigh.y,
            standardize=True
        )
        res = dde.utils.apply(mfnn3, (data, lay, wid,))
        ape.append(res[:2])
        y.append(res[2])

    predict = []
    for i in range(len(dataexp.X)):
        predict.append(res.predict(dataexp.X[i]))
        model, _ = dde.utils.apply(mfnn3, (data, lay, wid,))
        predict = [model.predict(dataexp.X[i]) for i in range(len(dataexp.X))]
        print(np.average(predict))

    print(np.average(predict))

def find_properties0(expname, train_hi, train_lo, lay=2, wid=128):
    datalow = FileData(train_lo, 'all')
    datahigh = FileData(train_hi, 'all')
    dataexp = FileData(expname, 'all', new=True)
    ...
    for train_index, _ in kf.split(larger.X):
        ...
        model, *metrics = dde.utils.apply(mfnn3, (data, lay, wid,))
        predict = [model.predict(dataexp.X[i]) for i in range(len(dataexp.X))]
        print(np.average(predict))
        ape.append(metrics[:2])
        y.append(metrics[2])
