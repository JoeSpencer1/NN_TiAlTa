...
'''
'''
categories = ['Physical', 'All 3D', 'Fine 3D', 'Coarse 3D', '2D FEM (4∠)', '2D FEM (70.3˚)']
σvalues = [10.319227, 6.0102615, 4.2227707, 13.366486, 63.699486, 59.360664]
σerrors = [3.7665966, 7.139462, 1.2720577, 13.127058, 39.098995, 73.83362]
Evalues = [3.4535072, 4.714753, 2.0597403, 1.2027538, 56.905693, 100.09694]
Eerrors = [1.8956615, 8.985055, 1.129313, 1.8326986, 45.235996, 112.177414]

colors = ['darkred', 'lightblue', 'green', 'gray', 'blue', 'darkorange']
bar_width = 0.15
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
ax1.bar(categories, σvalues, yerr=σerrors, capsize=4, color=colors)
...
categories = ['Physical', 'All 3D', 'Fine 3D', 'Coarse 3D', '2D FEM (4∠)', '2D FEM (70.3˚)']
σvalues = [10.667567, 388.7212, 556.4979, 1787.3728, 124.29811, 94.961464]
σerrors = [4.947879, 245.83269, 701.87775, 1157.989, 98.34832, 85.76816]
Evalues = [3.6597977, 144.05302, 221.13496, 743.5117, 16.04507, 24.682934]
Eerrors = [2.9540114, 89.383156, 107.329185, 410.12277, 4.6217675, 13.9773035]
...
Blx = [0.5, 0.25, 0.125]
Bly = [3.865413835269498e-05, 2.0172101147164164e-05, 5.0430252867910384e-06]
Bfx = [0.01, 1.5]
C30x = [0.1, 0.050, 0.025]
C30y = [1.72366466777758e-05, 3.339010727771235e-06, 8.347526819428091e-07]
C45x = [0.1, 0.050, 0.025]
...
Cf70 = equation(C70x, C70y, Cfx)
Bfq = equation(Bqx, Bqy, Bfx)
Bfl = equation(Blx, Bly, Bfx)

fig, ax = plt.subplots()
ax.scatter(C30x, C30y, color='blue', marker='s', label='30˚: $||e||_{L_{2}}=(2.63\\times 10 ^{-3})h^{2.18}$')
ax.scatter(C45x, C45y, color='red', marker='o', facecolor='none', label='45˚: $||e||_{L_{2}}=(2.41\\times 10 ^{-3})h^{2.16}$')
ax.scatter(C60x, C60y, color='green', marker='x', label='60˚: $||e||_{L_{2}}=(3.53\\times 10 ^{-3})h^{2.27}$')
...
ax.plot(Cfx, Cf60, linestyle="--", color='green')
...
ax.set_yscale('log')
ax.set_ylim([0.005*10.0**-5, 3*10.0**-5])
ax.grid(False)
...
ax.set_ylabel('L$_{2}$ error')
ax.set_yscale('log')
ax.set_ylim([0.08*10.0**-5, 7*10.0**-5])
ax.grid(False)
leg = ax.legend(loc='lower right')
...
ax.scatter(Blx, Bly, color='blue', marker='s', label='Linear: $||e||_{L_{2}}=(0.11 \\times 10^{-3}) h^{1.47}$')
ax.scatter(Bqx, Bqy, color='red', marker='o', facecolor='none', label='Quadratic: $||e||_{L_{2}}=(0.20\\times 10^{-3})h^{2.16}$')
ax.scatter(C70x, C70y, color='black', marker='^', label='70.0˚: $||e||_{L_{2}}=(0.15\\times 10 ^{-3})h^{1.70}$')
ax.plot(Bfx, Bfl, linestyle="--", color='blue')
ax.plot(Bfx, Bfq, linestyle="--", color='red')
ax.plot(Cfx, Cf70, linestyle="--", color='black')
ax.plot()
...
ax.set_ylabel('L$_{2}$ error')
ax.set_yscale('log')
ax.set_ylim([0.05*10.0**-6, 5*10.0**-3])
ax.grid(False)
leg = ax.legend(loc='lower right')
leg.set_title('Element order')
...
# 2D mesh
n_2D = [5, 10, 15, 20]
conσ = [1258.4299, 658.6289, 955.61487,532.2654]
εconσ = [913.89105, 521.97626, 487.05316, 240.66104]
conE = [158.34677, 110.91323, 67.60109, 60.02992]
εconE = [120.81767, 59.74704, 50.000793, 28.60466]
# 3D linear elemetns
n_3Dl = [5, 10, 15, 20]
l3Dσ = [802.34753, 598.2042, 412.33698, 593.4438]
εl3Dσ = [744.71545, 327.88074, 303.2981, 393.86002]
l3DE = [305.0752, 125.05515, 51.296722, 76.31165]
εl3DE = [273.73383, 100.19587, 44.35268, 59.636272]
# 3D quad elements
n_3Dq = [5, 10, 15, 20]
q3Dσ = [948.6102, 693.94135, 587.1541, 714.6778]
εq3Dσ = [1352.8092, 226.60805, 343.06134, 514.5537]
q3DE = [193.163, 246.93584, 169.33572, 153.87508]
εq3DE = [75.74496, 68.1138, 67.68839, 52.5883]
# Experimental data
n_exp = [5, 10, 15, 20]
expσ = [45.766357, 10.986177, 6.048375, 7.580364]
εexpσ = [16.797005, 8.63708, 3.120552, 6.1586657]
expE = [16.4799, 3.6843224, 1.1845193, 1.2331098]
εexpE = [13.894713, 2.9889755, 0.47664863, 0.55196935]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
ax1.errorbar(n_exp, expσ, yerr = εexpσ, color = 'green', linestyle = ':', label = "Experimental data")
ax1.errorbar(n_3Dq, q3Dσ, yerr = εq3Dσ, color = 'orange', linestyle = '--',label = "Quadratic 3D mesh")
ax1.errorbar(n_3Dl, l3Dσ, yerr = εl3Dσ, color = 'red',label = "Linear 3D mesh")
ax1.errorbar(n_2D, conσ, yerr = εconσ, color = 'blue',linestyle = '-.', label = "Linear 2D mesh")
ax1.set_yscale('log')
ax1.set_ylim([1, 3000])
ax1.set_xlim([-0.5, 21])
ax1.set_xticks([0, 5, 10, 15, 20])
...
ax2.errorbar(n_exp, expE, yerr = εexpE, color = 'green', linestyle = ':', label = "Experimental data")
ax2.errorbar(n_3Dq, q3DE, yerr = εq3DE, color = 'orange', linestyle = '--',label = "Quadratic 3D mesh")
ax2.errorbar(n_3Dl, l3DE, yerr = εl3DE, color = 'red',label = "Linear 3D mesh")
ax2.errorbar(n_2D, conE, yerr = εconE, color = 'blue', linestyle = '-.', label = "Linear 2D mesh")
ax2.set_yscale('log')
ax2.set_ylim([0, 600])
ax2.set_xlim([-0.5, 21])
ax2.set_xticks([0, 5, 10, 15, 20])
...
ax2.annotate("B: $E_{r}$", xy=(0.05, 0.95), xycoords="axes fraction",
              fontsize=12, ha="center",
              bbox=dict(boxstyle="round,pad=0.3", edgecolor="black", facecolor="lightgray"))
plt.savefig("/Users/Joe/Desktop/NN_graphs/sourcecompare.jpeg", dpi=800, bbox_inches="tight")
plt.show()
'''
# My MINN with all 3 data
n = [5, 10, 15, 20]
expσ = [37.298103, 15.402829, 6.779195, 5.868459]
εexpσ = [20.259314, 13.12174, 4.013392, 2.6640985]
expE = [22.370998, 2.446479, 1.1255435, 1.5920229]
εexpE = [30.051657, 1.328074, 0.33995497, 0.6270381]
# 2D data
conσ = [26.18924, 8.85075, 4.6805587, 3.735553]
εconσ = [24.69131, 3.309531, 1.6431087, 2.47793]
conE = [14.852114, 2.669698, 1.1249777, 1.1290767]
εconE = [15.289626, 0.93443465, 0.6047558, 0.62691253]
# 3D linear data
l3Dσ = [34.04951, 7.722165, 4.5092707, 2.7614205]
εl3Dσ = [31.69895, 5.567968, 2.9547353, 3.5373683]
l3DE = [12.821581, 2.9655967, 1.7589811, 0.7438924]
εl3DE = [6.5823374, 1.522445, 1.1243739, 0.37641913]
# 3D quadratic data
q3Dσ = [11.144818, 6.4337807, 2.4770777, 1.7518362]
εq3Dσ = [5.6005425, 2.6119251, 1.1069429, 1.1637901]
q3DE = [6.801137, 2.0571244, 1.3869501, 0.7662939]
εq3DE = [2.6278772, 1.3783776, 0.6426117, 0.37778568]

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
ax1.errorbar(n, expσ, yerr = εexpσ, color = 'green', label = "experimental data")
ax1.errorbar(n, conσ, yerr = εconσ, color = 'red', linestyle = '--', label = "2D mesh")
ax1.errorbar(n, l3Dσ, yerr = εl3Dσ, color = 'orange', linestyle = '-.',label = "linear 3D mesh")
ax1.errorbar(n, q3Dσ, yerr = εq3Dσ, color = 'blue', linestyle = ':',label = "quadratic 3D mesh")
ax1.set_yscale('linear')
ax1.set_ylim([1, 60])
ax1.set_xlim([-0.5, 21])
ax1.set_xticks([0, 5, 10, 15, 20])
ax1.set_yticks([10, 20, 30, 40, 50, 60])
ax1.set_yticklabels([10, 20, 30, 40, 50, 60])
ax1.legend()
ax1.set_ylabel("MAPE (%)")
ax1.set_xlabel("Experimental training data size")
ax1.annotate("A: $\sigma_{y}$", xy=(0.05, 0.95), xycoords="axes fraction",
...
ax2.errorbar(n, expE, yerr = εexpE, color = 'green', label = "Room temperature (n$_{tr}$=25)")
ax2.errorbar(n, conE, yerr = εconE, color = 'red', linestyle = '--', label = "Room temperature (n$_{tr}$=25)")
ax2.errorbar(n, l3DE, yerr = εl3DE, color = 'orange', linestyle = '-.', label = "Room temperature (n$_{tr}$=25)")
ax2.errorbar(n, q3DE, yerr = εq3DE, color = 'blue', linestyle = ':', label = "Room temperature (n$_{tr}$=25)")
ax2.set_yscale('linear')
ax2.set_ylim([0, 40])
ax2.set_xlim([-0.5, 21])
ax2.set_xticks([0, 5, 10, 15, 20])
...
ax2.annotate("B: $E_{r}$", xy=(0.05, 0.95), xycoords="axes fraction",
              fontsize=12, ha="center",
              bbox=dict(boxstyle="round,pad=0.3", edgecolor="black", facecolor="lightgray"))
plt.savefig("/Users/Joe/Desktop/NN_graphs/datacompare.jpeg", dpi=800, bbox_inches="tight")
plt.show()