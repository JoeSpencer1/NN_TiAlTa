...
def validation_two(yname, train_size, exp, low, hi, lay=2, wid=128):
    ...
    if train_size == 0:
        for train_index, test_index in kf.split(datalow.X):
            ...
            X_train, X_test = datalow.X[train_index], datahigh.X[test_index]
            y_train, y_test = datalow.y[train_index], datahigh.y[test_index]

            data = dde.data.DataSet(
                X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test
            )

            ape.append(dde.apply(nn, (data,lay,wid)))
    else:
        kf = ShuffleSplit(
