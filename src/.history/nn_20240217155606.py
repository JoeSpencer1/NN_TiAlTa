...
from data import BerkovichData, FEMData, ExpDataT, ExpData, FEM1, FEMData0
...
def validation_mf(yname, train_size, dlow, dhigh, dexp):
    datalow = FEM1(yname, dlow)
    datahigh = FEM1(yname, dhigh)
    dataexp = FEM1(yname, dexp)
...
def validation_exp_cross2(yname, train_size, dlow, dhig, dexp1, dexp2, typ='err'):
    datalow = FEM1(yname, dlow)
    dataBerkovich = FEM1(yname, dhig)
    dataexp1 = FEM1(yname, dexp1)
    dataexp2 = FEM1(yname, dexp2)
    ...
    with open('Output.txt', 'a') as f:
        f.write("cross2 raw " + dhig + ' ' + dlow + ' ' + dexp1 + ' ' + dexp2 + yname + ' ' + str(train_size) + ' ' + str(np.mean(y)) + ' ' + str(np.std(y)) + '\n')
