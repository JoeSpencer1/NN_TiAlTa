...
def main():
    ...
    for i in range(len(df)):
        if i not in [0,4,5,8,9,11,12,15,18,21,23,24,25]: # Remove indices that do not converge
            print(i, model_chollacoop(df['C (GPa)'][i], df['Wp/Wt'][i], df['dP/dh (N/m)'][i], df['nu'][i], df['hmax(um)'][i]))
    print('here')
    # print('Dao')
    # for i in range(len(df)):
