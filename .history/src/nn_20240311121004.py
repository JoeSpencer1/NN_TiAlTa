...
def validation_one(yname, testname, trainname, n_hi, lay=2, wid=32):
    ...
    with open('output.txt', 'a') as f:
        f.write('validation_one ' + yname + ' ' + str(n_hi) + ' ' + str(np.mean(mape)) + ' ' + str(np.std(mape)) + ' ' + t2s(testname) + ' ' + t2s(trainname) + ' ' + str(lay) + ' ' + str(wid) + '\n')
...
def validation_two(yname, testname, trainhigh, n_hi, trainlow, n_lo, lay=2, wid=128):
    ...
    with open('Output.txt', 'a') as f:
        f.write('validation_two ' + yname + ' ' + str(n_hi) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + ' ' + t2s(testname) + ' ' + t2s(trainhigh) + ' ' + t2s(trainlow) + ' ' + str(lay) + ' ' + str(wid) + '\n')
    print(np.std(mape))
    print(mape)
    print(yname, 'validation_two ', t2s(trainlow), ' ', t2s(trainhigh), ' ', str(n_hi), ' ', np.mean(mape), np.std(mape))

def validation_three(yname, n_hi, testname, trainexp, trainhigh, n_lo,trainlow, lay=2, wid=128):
    datalow = FileData(trainlow, yname)
    datahigh = FileData(trainhigh, yname)
