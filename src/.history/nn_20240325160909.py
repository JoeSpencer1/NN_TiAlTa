...
def validation_one(yname, testname, trainname, n_hi, n_vd=5, lay=2, wid=32):
    datatrain = FileData(trainname, yname)
    datatest = FileData(testname, yname)
    v_ind = np.random.choice(datatest.X.shape[0], size=n_vd, replace=False)
    t_ind = np.setdiff1d(np.arrange(datatest.X.shape[0]), v_ind)
    dataval = FileData(testname, yname)
    dataval.X = datatest.X[v_ind]
    dataval.y = datatest.y[v_ind]
    datatest.X = datatest.X[t_ind]
    datatest.y = datatest.y[t_ind]

    kf = ShuffleSplit(
        n_splits=10, train_size=n_hi, test_size=len(datatest.X) - n_hi, random_state=0
    )
    if n_hi == 0:
...
def validation_two(yname, testname, trainhigh, n_hi, trainlow, n_lo, v_lo=0, n_vd=5, lay=2, wid=128):
...
def validation_three(yname, testname, trainexp, n_exp, trainhigh, n_hi, trainlow, n_lo, typ='hi', n_vd=5, v_lo=0, v_hi=0, lay=2, wid=128):
    datalow = FileData(trainlow, yname)
    datahigh = FileData(trainhigh, yname)
