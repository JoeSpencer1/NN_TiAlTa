...
def validation_one(yname, trname, tstname, type, train_size, lay=9, wid=32, angles=[]):
...
def validation_two(yname, trname1, trname2, tstname, type, train_size, lay=9, wid=32, angles=[]):
    
    data1 = []
    if type == 'FEM':
        data1 = FEMDataT(yname, angles)
    if type == 'Berk':
        data1 = BerkovichDataT(yname)
    if type == 'Exp':
        data1 = ExpDataT(trname1, yname)
    data2 = ExpDataT(trname2, yname)
    tdata = ExpDataT(tstname, yname)
    
    if train_size[0] == 80:
        kf = RepeatedKFold(n_splits=5, n_repeats=2, random_state=0)
    elif train_size[0] == 90:
        kf = KFold(n_splits=10, shuffle=True, random_state=0)
    else:
        kf = ShuffleSplit(
            n_splits=10, test_size=len(data1.X) - train_size[0], random_state=0
        )

    mape = []
    iter = 0
    for train_index, test_index in kf.split(data1.X):
        iter += 1
        print('\nCross-validation iteration: {}'.format(iter))

        print(data1.X)
        X_train, X_test = data1.X[train_index], tdata.X[test_index]
        y_train, y_test = data1.y[train_index], tdata.y[test_index]

        dataa = dde.data.DataSet(
            X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test
        )

        mape.append(dde.utils.apply(nn, (dataa, lay, wid, )))
    ...
    for train_index, test_index in kf.split(data2.X):
        ...
        datab = dde.data.DataSet(
            X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test
        )

        mape.append(dde.utils.apply(nn, (datab, lay, wid, )))

    print(mape)
    print(yname, 'validation_one ', trname1, ' ', trname2, ' ' , tstname, ' ', str(train_size), ' ', np.mean(mape), ' ', np.std(mape))
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + trname1 + ' ' + trname2 + ' ' +  tstname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(train_size) + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')
