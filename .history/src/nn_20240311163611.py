...
def validation_three(yname, testname, trainexp, n_exp, trainhigh, n_hi, trainlow, n_lo, lay=2, wid=128):
    ...
    datalow.X = datalow.X[np.random.choice(datalow.X.shape[0], size=n_lo, replace=False)]
    datalow.y = datalow.y[np.random.choice(datalow.y.shape[0], size=n_lo, replace=False)]
    datahigh.X = datahigh.X[np.random.choice(datahigh.X.shape[0], size=n_hi, replace=False)]
    datahigh.y = datahigh.y[np.random.choice(datahigh.y.shape[0], size=n_hi, replace=False)]
