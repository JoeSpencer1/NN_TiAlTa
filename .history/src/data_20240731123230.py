...
class FileData(object):
    ...
    def read(self, name, new):
        ...
        elif self.yname == 's0.003':
            if 'Lu' in name or 'TI33' in name or '2D' in name or '3D' in name:
                df['s0.003'] = s003(df['Er (GPa)'].values, df['sy (GPa)'].values, df['n'].values)
            if self.y is None:
                self.y = df['s0.003'].values[:, None]
            else:
                self.y = np.vstack((self.y, df['s0.003'].values[:, None]))
