...
def validation_three(yname, train_size, train, exp, lay=2, wid=128):
    ...
    if train_size == 0:
        ...
        kf = ShuffleSplit(n_splits=10, train_size=train_size, random_state=0)
        for _ in range(10):
            index = np.random.choice(datatrain.y.flatten()))
            datatrain.X = np.delete(datatrain.X, index)
            datatrain.y = np.delete(datatrain.y, index)
        while len(datatrain.X) < train_size:
            datatrain.X = np.vstack((datatrain.X, datatrain.X))
            datatrain.y = np.vstack((datatrain.y, datatrain.y))
