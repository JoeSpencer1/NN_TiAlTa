...
def validation_three(yname, train_size, train, exp, lay=2, wid=128):
    ...
    kf = ShuffleSplit(n_splits=10, train_size=10, random_state=0)
    if train_size == 0:
        if len(dataexp.X) < len(dataBerk.X):
            short = dataexp
        else:
            short = dataFEM
        for train_index, _ in kf.split(short.X):
        ...
        for train_index, _ in kf.split(short.X):
            ...
            data = dde.data.MfDataSet(
                X_lo_train=dataFEM.X,
                X_hi_train=np.vstack((dataexp.X[train_index], dataBerk.X)),
                y_lo_train=dataFEM.y,
                y_hi_train=np.vstack((dataexp.y[train_index], dataBerk.y)),
                X_hi_test=dataexp.X,
                y_hi_test=dataexp.y,
                standardize=True
            )
    ...
    with open('Output.txt', 'a') as f:
        ...
        f.write('validation_two [' + train + ', ' + exp + '] ' + str(train_size) + ' ' + exp + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + str(np.mean(ape, axis=0)[0]) + ' ' + str(np.std(ape, axis=0)[0]) + '\n')
''''''
