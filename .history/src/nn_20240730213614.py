...
def validation_two(yname, testname, trainhigh, n_hi, trainlow, n_lo, v_lo=0, n_vd=0.2, lay=2, wid=128):
    ...
    if n_hi == 0:
        ...
        for train_index, test_index in kf.split(datatest.X):
            ...
            mape.append(dde.utils.apply(mfnn, (data, lay, wid, 1, ))[0])
    ...
    if n_exp == 0:
        ...
        if typ == 'hi':
            for train_index, test_index in kf.split(datatest.X):
                ...
                ape.append(dde.utils.apply(mfnn, (data, lay, wid, 1, ))[0])
        else:
            for train_index, test_index in kf.split(datatest.X):
                iter += 1
                print('\nCross-validation iteration: {}'.format(iter))
        ...
        for train_index, _ in kf.split(dataexp.X):
            ...
            res = dde.utils.apply(mfnn, (data, lay, wid, 1, ))
            ape.append(res[:2])
            y.append(res[2])
...
def find_properties(expname, train_hi, train_lo, lay=2, wid=128):
    datalowE = FileData(train_lo,'Er')
    datahighE = FileData(train_hi,'Er')
    dataexpE = FileData(expname,'Er', new=True)
    datalowsigma = FileData(train_lo,'sy')
    datahighsigma = FileData(train_hi,'sy')
    dataexpsigma = FileData(expname,'sy', new=True)
    datalown = FileData(train_lo,'n')
    datahighn = FileData(train_hi,'n')
    dataexpn = FileData(expname,'n', new=True)
...
#    for _ in range(10):
    for _ in range(1):
        print("\nIteration: {}".format(len(apeEr)))

        data = dde.data.MfDataSet(
            X_lo_train=datalowE.X,
            X_hi_train=datahighE.X,
            y_lo_train=datalowE.y,
            y_hi_train=datahighE.y,
            X_hi_test=dataexpE.X,
            y_hi_test=dataexpE.y,
            standardize=True
        )
        ...
        data = dde.data.MfDataSet(
            X_lo_train=datalowsigma.X,
            X_hi_train=datahighsigma.X,
            y_lo_train=datalowsigma.y,
            y_hi_train=datahighsigma.y,
            X_hi_test=dataexpsigma.X,
            y_hi_test=dataexpsigma.y,
            standardize=True
        )
        res = dde.utils.apply(mfnn, (data, lay, wid, 3,)) 
        apesy.append(res[:2])
        ...
        data = dde.data.MfDataSet(
            X_lo_train=datalown.X,
            X_hi_train=datahighn.X,
            y_lo_train=datalown.y,
            y_hi_train=datahighn.y,
            X_hi_test=dataexpn.X,
            y_hi_test=dataexpn.y,
            standardize=True
        )
        res = dde.utils.apply(mfnn, (data, lay, wid, 3,)) 
        apen.append(res[:2])
    ...
    print(np.mean(np.mean(Er, axis=0), axis=0))
    print(np.mean(np.std(Er, axis=0), axis=0))
    print(np.mean(np.mean(sy, axis=0), axis=0))
    print(np.mean(np.std(sy, axis=0), axis=0))
    print(np.mean(np.mean(n, axis=0), axis=0))
    print(np.mean(np.std(n, axis=0), axis=0))
