...
def validation_two(yname, trnames, tstname, type, train_size, lay=9, wid=32, angles=[]):
    ...
    if type == 'Exp':
        data1 = ExpDataT(trnames[0], yname)
    tdata = ExpDataT(tstname, yname)
    ...
    for i in range(1,len(trnames)):

        data2 = ExpDataT(trnames[1], yname)
    ...
    with open('Output.txt', 'a') as f:
        f.write('validation_one ' + trnames + ' ' +  tstname + ' ' + yname + ' ' + str(lay) + ' ' + str(wid) + ' ' + train_size + ' ' + str(np.mean(mape, axis=0)) + ' ' + str(np.std(mape, axis=0)) + '\n')
