...
def datastack(data, newsize, ogsize):
    if isinstance(newsize, tuple):
        start = 0
        newdata = FileData(None, data.yname)
        for i in range(len(newsize)):
            indices = np.random.choice(range(start, start+ogsize[i]), size=newsize[i], replace=False)
            newdata.X.append(data.X[indices])
            newdata.y.append(data.y[indices])
            start += newsize[i]
        return data
    else:
        newdata = data
...
def validation_one(yname, testname, trainname, n_hi, og_hi=0, lay=2, wid=32):
    datahigh = FileData(trainname, yname)
    ...
    if isinstance(n_hi, tuple):
        ...
        hi_total = n_hi
    kf = ShuffleSplit(
        n_splits=10, test_size=len(datatest.X) - hi_total, random_state=0
    )

    mape = []
    iter = 0
    print('datahigh', datahigh.y)
    tdatahigh = datastack(datahigh, n_hi, og_hi)
    print('datahigh', datahigh.y)
    print('tdatahigh', tdatahigh.y)
    while len(tdatahigh.X) < len(datahigh.X):
        temp = datastack(datahigh, n_hi, og_hi)
        tdatahigh.X = np.vstack(tdatahigh.X,temp.X)
        tdatahigh.y = np.vstack(tdatahigh.y,temp.y)
    for train_index, test_index in kf.split(tdatahigh.X):
        iter += 1
        print('\nCross-validation iteration: {}'.format(iter))

        X_train, X_test = tdatahigh.X[train_index], datatest.X[test_index]
        y_train, y_test = tdatahigh.y[train_index], datatest.y[test_index]
    ...
    print(mape)
    print(yname, hi_total, np.mean(mape), np.std(mape))
...
def validation_two(yname, testname, trainhigh, n_hi, trainlow, n_lo, og_hi=0, og_lo=0, lay=2, wid=128, len = [25]):
    ...
    tdatahigh = datastack(datahigh, n_hi, og_hi)
    tdatalow = datastack(datalow, n_lo, og_lo)
    print('tdatahigh', tdatahigh.X)
    print('datahigh', datahigh.X)
    print('tdatalow', tdatalow.X)
    while len(tdatahigh.X) < len(datahigh.X):
        temp = datastack(datahigh, n_hi, og_hi)
        tdatahigh.X = np.vstack(tdatahigh.X,temp.X)
        tdatahigh.y = np.vstack(tdatahigh.y,temp.y)
    while len(tdatalow.X) < len(datahigh.X):
        temp = datastack(datahigh, n_hi, og_hi)
        tdatalow.X = np.vstack(tdatalow.X,temp.X)
        tdatalow.y = np.vstack(tdatalow.y,temp.y)
    if hi_total == 0:
        for _ in range(10):
            data = dde.data.DataSet(
                X_train=tdatalow.X, y_train=tdatalow.y, X_test=datatest.X, y_test=datatest.y, standardize=True
            )
            mape.append(dde.utils.apply(nn, (data,lay,wid)))

    else:
        kf = ShuffleSplit(
            n_splits=10, test_size=len(tdatahigh.y) - hi_total, random_state=0
        )

        for train_index, test_index in kf.split(tdatahigh.X):
            iter += 1
            print('\nIteration: {}'.format(iter), flush=True)
            train_index = train_index % len(tdatahigh.X)
            test_index = test_index % len(datatest.X)
            
            data = dde.data.MfDataSet(
                X_lo_train=tdatalow.X[np.random.choice(tdatalow.X.shape[0], size=n_lo, replace=False)],
                X_hi_train=tdatahigh.X[train_index],
                y_lo_train=tdatalow.y[np.random.choice(tdatalow.y.shape[0], size=n_lo, replace=False)],
                y_hi_train=tdatahigh.y[train_index],
